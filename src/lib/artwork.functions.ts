...
// ── Regenerate album artwork for AzuraCast/Radio Uppsala defaults ─────────
// Targets albums whose artwork_path still lives under `/azuracast/`. For each
// album we pick one linked track title for verified search (iTunes → Deezer →
// MusicBrainz/Cover Art Archive), then AI fallback. Updates the album and
// mirrors the new cover to all of its linked tracks. AI-generated covers and
// already-replaced covers live on different paths and are skipped.

export const bulkRegenerateAzuracastAlbumArtwork = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => RegenerateInput.parse(input ?? {}))
  .handler(async ({ data, context }): Promise<RegenerateResult> => {
    if (!(await isAdmin(context.userId))) throw new Error("Endast admin");
    return (await sweepAzuracastAlbums(data.limit ?? 200)) as RegenerateResult;
  });

// ── Regenerate ALL single-release artwork (admin only) ────────────────────
// Refreshes covers for every album of type "single": iTunes → Deezer →
// MusicBrainz/Cover Art Archive → AI fallback. Updates both the single album
// cover and its linked track artwork. Overwrites existing covers.

export const bulkRegenerateSingleArtwork = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => RegenerateInput.parse(input ?? {}))
  .handler(async ({ data, context }): Promise<RegenerateResult> => {
    if (!(await isAdmin(context.userId))) throw new Error("Endast admin");
    ...
    for (const album of list) {
      ...
      // Linked tracks: use a track title for verification, fall back to album title.
      ...
      try {
        ...
        if (!path) {
          ...
          if (upErr) {
            ...
            // Mirror the cover onto every linked track.
...
export const bulkRegenerateTrackArtwork = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<RegenerateResult> => {
    if (!(await isAdmin(context.userId))) throw new Error("Endast admin");
    return (await sweepAzuracastTracks(data.limit ?? 200)) as RegenerateResult;
  });
